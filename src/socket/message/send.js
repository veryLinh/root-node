...
const handlerMessage = require('../message');
...
const makeMessagesSocket = (config) => {
  ...
  const client = new handlerMessage(Utils_1, waUploadToServer, relayMessage);
  ...
  return {
    ...
    sendMessage: async (jid, content, options = {}) => {
      ...
      if (typeof content === 'object' && 'album' in content && content.album) {
        ...
        return albumMsg;
      } else if (client.detectType(content) === 'GROUP_STORY') {
        const {
          quoted
        } = options;
        return await client.handleGroupStory(content, jid, quoted);
      } else {
        let mediaHandle;
