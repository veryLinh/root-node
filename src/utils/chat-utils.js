...
const LabelAssociation_1 = require("../core/Types/LabelAssociation");
...
const decodeSyncdSnapshot = async (name, snapshot, getAppStateSyncKey, minimumVersionNumber, validateMacs = true) => {
  ...
  const {
    hash,
    indexValueMap
  } = await (0, exports.decodeSyncdMutations)(snapshot.records, newState, getAppStateSyncKey, areMutationsRequired ?
    (mutation) => {
      var _a;
...
const processSyncAction = (syncAction, ev, me, initialSyncOpts, logger) => {
  ...
  if (action === null || action === void 0 ? void 0 : action.muteAction) {
    ...
    ev.emit('contacts.upsert', [{
      ...
      jid: (0, WABinary_1.isJidUser)(id) ? id : action.contactAction.pnJid
    }]);
