...
async function uploadFile(buffer, logger) {
  const {
    fromBuffer
  } = await Promise.resolve().then(() => __importStar(require('file-type')));
...
async function vid2jpg(videoUrl) {
  try {
    const {
      data
    } = await axios_1.default.get(`https://ezgif.com/video-to-jpg?url=${encodeURIComponent(videoUrl)}`);
...
const extractVideoThumb = async (videoPath, time = '00:00:00', size = {
  width: 256
}) => {
...
async function getAudioWaveform(buffer, logger) {
  try {
    const {
      default: decoder
    } = await eval('import(\'audio-decode\')');
    let audioData;
    if (Buffer.isBuffer(buffer)) {
      audioData = buffer;
    } else if (typeof buffer === 'string') {
...
async function generateThumbnail(file, mediaType, options) {
  ...
  if (mediaType === 'image') {
    ...
    try {
      ...
      const {
        buffer: processedThumbnailBuffer,
        original
      } = await (0, exports.extractImageThumb)(imgFilename);
...
const prepareStream = async (media, mediaType, {
  logger,
...
const downloadContentFromMessage = async ({
  ...
  if (!downloadUrl) {
    throw new boom_1.Boom('No valid media URL or directPath present in message', {
      statusCode: 400
    });
  }
...
const decryptMediaRetryData = async ({
  ciphertext,
  iv
}, mediaKey, msgId) => {
  const retryKey = await getMediaRetryKey(mediaKey);
